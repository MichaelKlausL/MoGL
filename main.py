...
def train(args):
    ...
    if isinstance(ckpt_path, str) and ckpt_path.strip() == "":
        ckpt_path = None
    if isinstance(ckpt_path, str) and ckpt_path.lower() in {"auto", "last"}:
        ckpt_path = "last"
    if ckpt_path is not None and isinstance(ckpt_path, str):
        if ckpt_path != "last" and not os.path.exists(ckpt_path):
